...
data <- 
  ...
  ## code a binomial factor to describe infection levels as infected vs uninfected
  mutate(Infection_binomial = ifelse(Infection_intensity == 0, 0, 1) %>%
                 as.factor()) %>% 
  ## code infection levels in three classes
...
First, we use a principal component analysis as unsupervised clustering approach

```{r}
## Subset the raw data for PCA
## "GLDH": Essentially uniform, below detection threshold
## "Basophiles_percent": extremely rare, two factor levels (0, 1)
pca.dat <- 
  data[,
...
The above plot shows that there are a few issues regarding collinear variables. Most evident is this for the ratio between Heterocytes and Lymphocytes.
...
```{r}
indinfplot <- ggplot(data = pca.coords, 
...
MVA.cv(pca.dat[buteo,], data$Infectionclass[buteo], model="LDA", scale=T)
