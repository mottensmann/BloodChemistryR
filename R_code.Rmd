...
### LDA: Species discrimination

Species are well disciminated by overall blood chemistry, while *Accipiter gentilis* samples show the highest within group variance.
...
lda <- lda(as.factor(Species) ~ ., data = vars, prior = rep(1, length(unique(vars$Species)))/length(unique(vars$Species)))

#Project data on linear discriminants
lda.values <- predict(lda, vars[,-1])

## variance explained
r.sq <- (lda$svd^2/sum(lda$svd^2))

## visualise loadings
# df.lda <- data.frame(varnames = rownames(coef(lda)), coef(lda))
# rad <- length(unique(vars$Species))
...
lda_plot <- ggplot(cbind(vars, lda.values$x),
  ...
  geom_point() +
  theme_classic() +
  xlab(paste0("LD2 (", round(r.sq[2] * 100, 2), "%)")) +
  ylab(paste0("LD1 (", round(r.sq[1] * 100, 2), "%)")) +
  theme(legend.position = "bottom") 
lda_plot
```

### LDA: Species and infection status

```{r}
vars <- data[,c("Species", "Leukocyte_count", "Lymphocytes_percent", "Monocytes_percent", "Heterophiles_percent", "Basophiles_percent", "Eosinophiles_percent",  "AP", "AST", "gGT", "CHE", "GLDH", "BA", "LDH", "CK", "ALB", "TP", "Infection_binomial")]

## leave out Accipiter gentilis, to few samples
vars <- filter(vars, Species %in% c("Buteo buteo", "Milvus milvus"))

vars[["Species"]] <- paste0(vars[["Species"]],"-L", vars[["Infection_binomial"]])
vars <- subset(vars, select = -c(Infection_binomial))
...
### LDA: Buteo buteo details

```{r}
vars <- data[,c("Species", "Leukocyte_count", "Lymphocytes_percent", "Heterophiles_percent", "Basophiles_percent", "Eosinophiles_percent",  "AP", "AST", "gGT", "CHE", "GLDH", "BA", "LDH", "CK", "ALB", "TP", "Infection_intensity")]

## leave out Accipiter gentilis, to few samples
vars <- filter(vars, Species %in% c("Buteo buteo"))
vars$Infection_intensity[vars$Infection_intensity == 0] <- "uninfected"
vars$Infection_intensity[vars$Infection_intensity %in% 1:2] <- "low"
vars$Infection_intensity[vars$Infection_intensity %in% 3:4] <- "high"

vars[["Species"]] <- paste0(vars[["Species"]],"---", vars[["Infection_intensity"]])
vars <- subset(vars, select = -c(Infection_intensity))
...
lda_plot
=======
>>>>>>> 9be62d77df38dc42770ba14b916f5f6403c1c74b
```
